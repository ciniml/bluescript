(() => {
  ...
  // src/toolchain-worker.ts
  init_buffer();
  init_process();
  var factories = /* @__PURE__ */ new Map();
  var persistentFiles = /* @__PURE__ */ new Map();
  var modules = /* @__PURE__ */ new Map();
  var base = new URL("./toolchain/bin/", self.location.href).href;
  function factoryOf(tool) {
    let f = factories.get(tool);
    if (!f) {
      importScripts(`${base}${tool}.js`);
      f = self.createTool;
      delete self.createTool;
  ...
  self.onmessage = async (ev) => {
    ...
    if ("register" in msg) {
      ...
      self.postMessage({ id: msg.id, code: 0, stdout: "", stderr: "", outputs: {} });
      return;
    }
    if ("warmup" in msg) {
    ...
    try {
      ...
      self.postMessage(res, Object.values(res.outputs).map((u) => u.buffer));
    } catch (e) {
      self.postMessage({ id: msg.id, code: 1, stdout: "", stderr: "", outputs: {}, error: String(e?.stack ?? e) });
    }
  };
})();
...
buffer/index.js:
   ...
   *)
*/
//# sourceMappingURL=toolchain-worker.js.map
