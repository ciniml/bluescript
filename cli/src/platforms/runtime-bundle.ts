...
import { RUNTIME_BUNDLE_MANIFEST, RuntimeBundleManifest, RuntimeBundleComponent, esp32BuildDirName, esp32StdModulePath, EspIdfComponents, parseEspAppDesc } from '@bscript/lang';
...
function packageComponents(bundleDir: string, buildDir: string, board: Esp32FamilyBoardName, options: RuntimeBundleOptions): Partial<RuntimeBundleManifest> {
    ...
    const idf = new EspIdfComponents(buildDir, options.espDir, esp32TargetOf(board));
    ...
    if (options.gccPath) {
        const candidates = [
            ...
            path.join(path.dirname(options.gccPath), '..', `xtensa-${esp32TargetOf(board)}-elf`, 'include'),
        ];
...
export function createRuntimeBundle(runtimeDir: string, board: Esp32FamilyBoardName, options: RuntimeBundleOptions = {}): string {
    ...
    const buildDir = path.join(portDir, esp32BuildDirName(board));
    ...
    copy(path.join(portDir, esp32StdModulePath(board, esp32TargetOf(board))), path.join(bundleDir, 'std-module.bs'));
